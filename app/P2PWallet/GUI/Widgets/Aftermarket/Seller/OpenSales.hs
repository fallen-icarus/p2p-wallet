...
openSalesWidget model@AppModel{knownWallets,aftermarketModel=AftermarketModel{..},reverseTickerMap,config} =
  ...
  where
    ...
    allSales = filter ( (==Just True) 
                      . fmap (\d -> is _SpotDatum d || is _AuctionDatum d)
                      . view #marketDatum
                      ) utxos
...
inspectSaleWidget :: AppModel -> AppNode
inspectSaleWidget model@AppModel{..} = do
    vstack
      [ centerWidget $ vstack
          [ centerWidgetH $ hstack
              ...
              , tooltip_ "Resync Info" [tooltipDelay 0] $
                  box_ [alignMiddle , onClick $ AftermarketEvent $ LookupKeyInfo (True,utxo)] $
                    label refreshIcon
                      `styleBasic` 
                        [ border 0 transparent
                        , radius 20
          ...
          , spacer
          , box_ [alignRight] $ 
              button "Close" (AftermarketEvent $ AftermarketSellerEvent CloseInspectedAftermarketSale)
  ...
  where
    utxo = fromMaybe def $ aftermarketModel ^. #sellerModel % #inspectedSale
    (policyId,names) = fromMaybe ("",[]) $ aftermarketUTxONfts utxo
    header = "Aftermarket NFT Batch"
    batchNode
      | policyId == Loans.activeBeaconCurrencySymbol = map (inspectLoanWidget model) names
      | policyId == Options.activeBeaconCurrencySymbol = map (inpsectOptionsWidget model) names
      | otherwise = map (inpsectUnknownNftWidget policyId) names

inspectLoanWidget :: AppModel -> TokenName -> AppNode
inspectLoanWidget AppModel{..} tokenName =
    hstack
      [ activeStatus loanUTxO
      , spacer_ [width 3]
      , box_ [alignCenter,alignMiddle] $ tooltip_ "Add to Home Batch" [tooltipDelay 0] $
          mainButton addIcon 
            (AftermarketEvent $ AftermarketSellerEvent $ AddNftToHomeBatch loanIdNft)
            `styleBasic` 
              [ textSize 10
              , textFont "Remix"
              , textMiddle
  ...
  where
    ...
    activeStatus u@LoanUTxO{loanAddress,utxoRef,lovelace=utxoLovelace,nativeAssets=utxoNativeAssets} = do
      let Loans.ActiveDatum{..} = fromMaybe def $ loanUTxOActiveDatum u
          ...
          loanHistoryEvt = AftermarketEvent $ AftermarketSellerEvent$ InspectSellerLoanHistory loanId
          lookupBorrowerEvt = AftermarketEvent 
                            $ AftermarketSellerEvent 
                            $ InspectSellerBorrowerInformation (borrowerId, loanAddress)
      vstack
        [ hstack
...
inpsectOptionsWidget :: AppModel -> TokenName -> AppNode
inpsectOptionsWidget AppModel{..} tokenName =
    hstack
      [ activeStatus optionsUTxO
      , spacer_ [width 3]
      , box_ [alignCenter,alignMiddle] $ tooltip_ "Add to Home Batch" [tooltipDelay 0] $
          mainButton addIcon 
            (AftermarketEvent $ AftermarketSellerEvent $ AddNftToHomeBatch contractIdNft)
            `styleBasic` 
              [ textSize 10
              , textFont "Remix"
              , textMiddle
...
inpsectUnknownNftWidget :: CurrencySymbol -> TokenName -> AppNode
inpsectUnknownNftWidget policyId tokenName =
    hstack
      [ assetRow
      , spacer_ [width 3]
      , box_ [alignCenter,alignMiddle] $ tooltip_ "Add to Home Batch" [tooltipDelay 0] $
          mainButton addIcon 
            (AftermarketEvent $ AftermarketSellerEvent $ AddNftToHomeBatch unknownNft)
            `styleBasic` 
              [ textSize 10
              , textFont "Remix"
              , textMiddle
  ...
  where
    unknownNft@NativeAsset{fingerprint} = mkNativeAsset policyId tokenName & #quantity .~ 1
    ...
    hstack
      ...
      , box_ [alignCenter,alignMiddle] $ tooltip_ "Remove Key NFT" [tooltipDelay 0] $
          button closeCircleIcon 
            (AftermarketEvent $ AftermarketSellerEvent $ RemoveSellerSaleUpdateNft nft)
            `styleBasic` 
              [ textSize 10
              , textColor customRed
              , textFont "Remix"
