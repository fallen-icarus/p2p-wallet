module P2PWallet.GUI.Widgets.TxBuilder.AftermarketBuilder.SaleUpdates
  ( 
    saleUpdatesList
...
saleUpdatesList :: ReverseTickerMap -> [(Int,SaleUpdate)] -> [AppNode]
saleUpdatesList reverseTickerMap = map utxoRow
  where
    ...
    utxoRow :: (Int,SaleUpdate) -> AppNode
    utxoRow s@(idx,SaleUpdate{oldSale,newSale=SaleCreation{..}}) = do
      let policyId = maybe "" (view #policyId) $ maybeHead nfts
          ...
          (newSaleType, priceHeader)
            | isAuction = ("Auction", "Starting Price:")
            | otherwise = ("Spot", "Sale Price:")
          oldSaleType
            | isJust $ oldSale ^? #marketDatum % _Just % _AuctionDatum = "Auction"
            | otherwise = "Spot"
      hstack
        [ vstack
            [ hstack
                [ label ("Update Aftermarket Sale (" <> oldSaleType <> " -> " <> newSaleType <> ")")
                    `styleBasic` [textSize 10, textColor customBlue]
                , spacer_ [width 5]
                , separatorLine `styleBasic` [fgColor darkGray, paddingT 1, paddingB 1]
                , spacer_ [width 5]
                , let prettyRef = display (oldSale ^. #utxoRef) in
                ...
                , spacer_ [width 5]
                , flip styleBasic [textSize 10] $ tooltip_ (oldSale ^. #walletAlias) [tooltipDelay 0] $
                    label userIcon
                      `styleBasic` 
                        [ textMiddle
                        , textFont "Remix"
        ...
        , hstack
            [ vstack
                ...
                , box_ [alignCenter,alignTop] $ tooltip_ "Edit Action" [tooltipDelay 0] $
                    button editIcon 
                        (aftermarketBuilderEvent $ EditSelectedSaleUpdate $ StartAdding $ Just s)
                      `styleBasic` 
                        [ textSize 10
                        , textColor customBlue
                        , textFont "Remix"
            ...
            , vstack
                ...
                , box_ [alignCenter,alignTop] $ tooltip_ "Remove Action" [tooltipDelay 0] $
                    button closeCircleIcon (aftermarketBuilderEvent $ RemoveSelectedSaleUpdate idx)
                      `styleBasic` 
                        [ textSize 10
                        , textColor customRed
                        , textFont "Remix"
...
editSaleUpdateWidget :: AppModel -> AppNode
editSaleUpdateWidget model = do
  let maybeLens' = maybeLens (0,def) (#txBuilderModel % #aftermarketBuilderModel % #targetSaleUpdate)
...
spotEditWidget model@AppModel{..} = do
  ...
  vstack
    ...
    , box_ [alignRight] $ 
        hstack
          [ button "Cancel" (aftermarketBuilderEvent $ EditSelectedSaleUpdate CancelAdding)
              `styleBasic` [textSize 10]
          , spacer
          , mainButton "Update" (aftermarketBuilderEvent $ EditSelectedSaleUpdate ConfirmAdding)
              `styleBasic` [textSize 10]
          ]
