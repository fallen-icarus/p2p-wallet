...
offerUpdatesList :: ReverseTickerMap -> TimeZone -> [(Int,OfferUpdate)] -> [AppNode]
offerUpdatesList reverseTickerMap timeZone = map utxoRow
  where
    ...
    utxoRow s@(idx,OfferUpdate{oldOffer,newOffer=oc@OfferCreation{alias,paymentWallet}}) = do
