module P2PWallet.GUI.Widgets.Lending
  ( 
    lendingWidget
  ) where
...
import P2PWallet.GUI.Widgets.Lending.Borrow
import P2PWallet.GUI.Widgets.Lending.Lend
import P2PWallet.GUI.Widgets.Lending.Research
import P2PWallet.Prelude

lendingWidget :: AppModel -> AppNode
lendingWidget model@AppModel{..} = do
    zstack
      ...
      , widgetIf isAdding $ addLoansWalletWidget model
      , widgetIf isDeleting $ confirmDeleteWidget model
      ]
  where
    hasLoanWallets :: Bool
    hasLoanWallets = knownWallets ^. #loanWallets /= []

    hasStakeWallets :: Bool
    hasStakeWallets = knownWallets ^. #stakeWallets /= []

    isAdding :: Bool
    isAdding = lendingModel ^. #addingWallet

    isDeleting :: Bool
    isDeleting = lendingModel ^. #deletingWallet
...
mainWidget model@AppModel{scene=_,..} =
    vstack
      ...
      , spacer_ [width 5]
      , borrowWidget model `nodeVisible` (lendingModel ^. #scene == BorrowScene)
  ...
  where
    loansMainSceneButton :: Text -> LendingScene -> AppNode
    loansMainSceneButton caption scene = do
      let dormantColor
            | lendingModel ^. #scene == scene = customBlue
            | otherwise = white
          hoverColor
            | lendingModel ^. #scene == scene = customBlue
            | otherwise = lightGray
      button caption (LendingEvent $ ChangeLendingScene scene)
    ...
    (walletTypeTip,walletTypeIcon)
      | isNothing $ lendingModel ^. #selectedWallet % #stakeKeyDerivation = ("Watched", watchedIcon)
      | otherwise = ("Paired", pairedIcon)

    mainSceneMenu :: AppNode
    mainSceneMenu = hstack 
      [ spacer
      , loansMainSceneButton "Borrow" BorrowScene
      , spacer
      , separatorLine `styleBasic` [paddingT 5, paddingB 5]
      , spacer
      , loansMainSceneButton "Lend" LendScene
      , spacer
      , separatorLine `styleBasic` [paddingT 5, paddingB 5]
      , spacer
      , loansMainSceneButton "Research" ResearchLoansScene
      , spacer
      , separatorLine `styleBasic` [paddingT 5, paddingB 5]
      , spacer
    ...
    walletMenu = do
      ...
      hstack
        ...
        , spacer_ [width 5]
        , textDropdown_ 
              (toLensVL $ #lendingModel % #selectedWallet) 
...
confirmDeleteWidget model = do
  centerWidget $ vstack_ [childSpacing]
    ...
    , centerWidgetH $ label $ mconcat
        ...
        , model ^. #lendingModel % #selectedWallet % #alias
        , "'?"
        ]
    , centerWidgetH $ label
        "This will only remove the wallet as a loan wallet."
          `styleBasic` [textSize 12, textColor customRed]
    , hstack 
        [ filler
        , button "Cancel" $ LendingEvent $ DeleteLoanWallet CancelDeletion
        , spacer
        , mainButton "Confirm" $ LendingEvent $ DeleteLoanWallet ConfirmDeletion
        ]
    ] `styleBasic` [bgColor customGray3, padding 20]

-- A notice that stake wallets must be added before loan wallets.
requiresFirstStakeWallet :: AppNode
requiresFirstStakeWallet =
  centerWidget $ vstack
    [ box_ [alignMiddle] $ label "You must add a stake wallet before you can borrower/lend."
...
addLoansWalletWidget :: AppModel -> AppNode
addLoansWalletWidget AppModel{knownWallets,lendingModel} = do
  let innerDormantStyle = 
      ...
      maybeLens' = maybeLens def (#lendingModel % #targetStakeCredential)
      targetStakeCredential = lendingModel ^. #targetStakeCredential
      currentLoanWalletIds = map (view #stakeWalletId) $ knownWallets ^. #loanWallets
  ...
  centerWidget $ vstack 
    ...
    -- This should only appear when there are tracked loan wallets but no stake wallets; all of the
    ...
    , hstack 
        [ filler
        , button "Cancel" $ LendingEvent $ AddNewLoanWallet CancelAdding
        , spacer
        , mainButton "Confirm" (LendingEvent $ AddNewLoanWallet ConfirmAdding)
  ...
  vstack
    [ centerWidget $
        flip styleBasic [bgColor transparent, padding 20, radius 5] $ 
          box $ 
            label "Add your first loan wallet to begin!" 
             `styleBasic` [textFont "Italics"]
    , filler
    , hstack
        [ filler
        , box (mainButton "Add Wallet" $ LendingEvent $ AddNewLoanWallet $ StartAdding Nothing) 
            `styleBasic` [padding 20]
        ]
    ]

-- A popup menu for actions on the Loans page.
morePopup :: AppNode
morePopup = do
  vstack
    [ -- This is the button that opens the popup.
      tooltip_ "More" [tooltipDelay 0] $
        box_ [onClick $ LendingEvent ShowLendingMorePopup] $ label horizontalMoreIcon
          `styleBasic`
            [ border 0 transparent
            , radius 20
            , paddingL 0
    ...
    , customPopup (toLensVL $ #lendingModel % #showMorePopup) $
        vstack
          [ button "Add Wallet" (LendingEvent $ AddNewLoanWallet $ StartAdding Nothing)
              `styleBasic`
                [ border 0 transparent
                , textSize 12
                , bgColor transparent
          ...
          , button "Delete Wallet" (LendingEvent $ DeleteLoanWallet $ GetDeleteConfirmation Nothing)
              `styleBasic`
                [ border 0 transparent
                , textSize 12
                , bgColor transparent
